...
describe("Contract Test", function () {
    ...
    it("Should Raise a Grievance & Other Resident Votes for it & After Sufficient Votes converted to Issue", async function(){
        ...
        const name3 = "Jane Doe", age3 = 20, email3 = "janedoe@gmail.com", location3 = "Kandivali";
        ...
        await expect(contract.connect(owner).addResident(name3, age3, email3, location3)).to.emit(contract, "AddedResident").withArgs(owner, name3, email3, location3);
        await expect(contract.connect(owner).addToDAO()).to.emit(contract, "AddedToDAO").withArgs(owner, 3);
        await expect(contract.connect(owner).addToDAO()).to.emit(contract, "AddedToDAO").withArgs(owner, 4);

        await expect(contract.connect(addr1).raiseGrievance(1, "ABC", ["ab", "cd"], "Message", "Location", ["img1", "img2"])).to.emit(contract, "RaisedGrievance").withArgs("ABC", "Message");
        
        await expect(contract.connect(addr2).voteOnGrievance(0)).to.emit(contract, "UpvotedAGrievance").withArgs(0, 1, 0);
    ...
    it("Should Add an Update on an Issue", async function(){
        ...
        await expect(contract.connect(addr1).addToDAO()).to.emit(contract, "AddedToDAO").withArgs(addr1, 1);
        await expect(contract.connect(addr1).raiseGrievance(1, "ABC", ["ab", "cd"], "Message", "Location", ["img1", "img2"])).to.emit(contract, "RaisedGrievance").withArgs("ABC", "Message");
        await expect(contract.connect(addr1).voteOnGrievance(0)).to.emit(contract, "UpvotedAGrievance").withArgs(0, 1, 1);
